...
public class WITSCommand {
    ...
    private static void listStructuresAtSpot(ServerLevel level, Coordinates coordinates, boolean callerPosition, CommandContext<CommandSourceStack> cs) {
        ...
        ChunkPos chunkPos = ChunkPos.containing(centerPos);

        List<StructureStart> structureStarts = level.structureManager().startsForStructure(chunkPos.x(), chunkPos.z(), _ -> true);
        ...
        if (callerPosition) {
            ...
            component = MutableComponent.create(PlainTextContents.create("Structure(s) at "))
                    .append(Component.translatable("chat.coordinates", centerPos.getX(), centerPos.getY(), centerPos.getZ()))
                    .append(":");
        }
        ...
        for (Structure structure : structures) {
            ...
            component.append(Component.literal("\n -").withStyle(ChatFormatting.RESET))
                    .append(ComponentUtils.copyOnClickText(key.toString()).withStyle(ChatFormatting.GOLD));
        }
